...
def main():
    ...
    setproctitle.setproctitle(f'{env_name}-SAC-{seed}')
    
    algo = SAC_continuous(
            state_shape=state_shape, action_shape=action_shape, device=device, seed=seed, gamma=gamma,
                 SAC_batch_size=SAC_batch_size, buffer_size=buffer_size, lr_actor=lr_actor, lr_critic=lr_critic, 
                 lr_alpha=lr_alpha, hidden_units_actor=hidden_units_actor, hidden_units_critic=hidden_units_critic, 
                 start_steps=start_steps,tau=tau,max_episode_length=max_episode_length, reward_factor=reward_factor,
                 max_grad_norm=max_grad_norm)
    eval_algo = deepcopy(algo)

    wandb.init(project=f'test-{env_name}', settings=wandb.Settings(_disable_stats=True), \
        group='test-SAC', name=f'{seed}', entity='hmhuy')
