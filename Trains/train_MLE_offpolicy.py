...
sys.path.append('..')
sys.path.append('./')
from Parameters.MLE_offpolicy import *
#------------------------------------------#
def main():
    ...
    #------------------------------------------#
    from Sources.algos.MLE import MLE_offpolicy
    ...
    def train_from_data(algo,eval_algo):
        t = np.array([0 for _ in range(num_envs)])
        eval_thread = None
        state,_ = env.reset()
        
        for step in range(num_training_step):
            if (step%100 == 0):
                print(f'train: {step/num_training_step*100:.2f}% {step}/{num_training_step}', end='\r')
            log_info = {'log_cnt':step//log_freq}
            algo.update_from_dataset(log_info)
    ...
    def train(env,test_env,algo,eval_algo):
        ...
        for step in range(1,num_training_step//num_envs+1):
            ...
            if algo.is_update(step*num_envs):
                    log_info = {'log_cnt':(step*num_envs)//log_freq}
                    algo.update(log_info)
                    if ((step*num_envs)%log_freq == 0):
                        print(log_info)
    ...
    setproctitle.setproctitle(f'{env_name}-MLE-{seed}')
    ...
    algo = MLE_offpolicy(expert_buffer=expert_buffer,noisy_buffer=noisy_buffer,state_shape=state_shape,
            action_shape=action_shape,device=device, seed=seed, gamma=gamma,buffer_size=buffer_size,
            hidden_units_actor=hidden_units_actor,hidden_units_critic=hidden_units_critic,
            lr_actor=lr_actor,lr_critic=lr_critic,max_grad_norm=max_grad_norm,reward_factor=reward_factor,
            SAC_batch_size=SAC_batch_size, lr_alpha=lr_alpha, start_steps=start_steps, tau=tau,
    ...
    train_from_data(algo=algo,eval_algo=eval_algo)
